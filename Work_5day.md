...
### W5-STABLE-APK-UI-BASELINE-AND-SURFACE-INPUT-BRANCH-20260926
- Preserved the verified GUI source point as baseline/stable-animated-ui-20260926 at commit 9ae7f33. Started feature/surface-input-resolution-20260926 from the same commit for further work. All pre-existing uncommitted device-tree and AOSP patch changes remain untouched on the development branch; none were included in this checkpoint commit. In the local QEMU source checkout, baseline/gtk-full-redraw-20260926 and feature/surface-input-resolution-20260926 both point to bde658e.
- The first attempted regression used the runner's stale default kernel/vendor pair and showed the inactive-display placeholder. That was not the recorded known-good configuration. Re-ran the existing image set without guest input devices: kernel-linux-7.3-rc4-4k-netfilter-20260925, system-kikiaosp-ui-keep-screen-on-20260926.img, vendor-kikiaosp-ui-scanout-pixel-count-20260926.img, and the already-used -AudioStubOutput option.
- The exact-pair run reached sys.boot_completed=1; Android logged Displayed com.kikiaosp.windowtest/.MainActivity at about 80 seconds guest uptime, with recurring WINQ-PRESENT flush through at least 119 seconds. The full Windows desktop captures at 2880x1920 show the Android Activity, Close button, TEST OK, and the moving blue block in different positions:
  - C:\Users\keke\Downloads\temp\qemu-desktop-gui-baseline-restored-20260926.png — SHA-256 c701188160f903f327c62f69b01ccf957fad8a79cd768755998807ca376e9bc2.
  - C:\Users\keke\Downloads\temp\qemu-desktop-gui-baseline-restored-frame2-20260926.png — SHA-256 e76e12e5ed870cf99acb7bb117aab34b19de292812d8fc4070637fd72c5ca0cf.
- The exact-pair log has no SurfaceFlinger SIGSEGV or guest suspend entry during the observed interval. QEMU was closed gracefully after capture and ports 4447/5555 were released.
- Removed the experimental -VirtioInput option and its automatic virtio-keyboard-pci / virtio-tablet-pci devices from the local Windows launch script. The input-enabled trial remains only as diagnostic logs; it is not the baseline and is not wired into normal startup. No kernel or Android rebuild was performed.

### W5-SURFACE-TOUCH-ADB-ROUTING-KEYBOARD-20260926
- On the isolated `feature/surface-input-resolution-20260926` branch, the Windows GTK QEMU frontend and a single `virtio-multitouch-pci` device booted the existing 7.3-rc4 4K kernel and Kiki Android system to the real animated `KikiAOSP test window` Activity. Full 2880x1920 desktop evidence: `C:/Users/keke/Downloads/temp/qemu-desktop-adb-netdiag-v3-full-ui-20260926.png`. The user tapped the Surface screen repeatedly while a bounded guest `getevent` capture ran. UART records 38 `BTN_TOUCH` transitions (19 DOWN/UP pairs), matching tracking-ID begin/end and changing MT coordinates in `qemu-kikiaosp-adb-netdiag-v3-20260926.log`. A later tap coordinate landed in the visible Close-button area, after which a full-desktop capture showed QEMU's inactive display placeholder; `MainActivity.java` confirms that button calls `finish()`. This supports input delivery to the Android UI, while exact Activity lifecycle correlation remains to be checked.
- The ADB offline cause was traced below `adbd`. The guest had `eth0=10.0.2.15/24`, `adbd` listening on TCP 5555, and IPv6 `bindv6only=0`, yet QEMU's hostfwd stayed in SYN_SENT. A runtime QEMU `filter-dump` capture (`aosp/windows-arm64-test/adb-netdiag-v3-20260926.pcap`) recorded repeated ARP queries from 10.0.2.2 to 10.0.2.15 without replies. Guest RX packet count grew from 1 to 6 during host attempts, while a guest ping to 10.0.2.2 returned `Network is unreachable`. Guest `ip rule show` ended in `32000: from all unreachable` and did not select the connected route in `main`.
- A targeted rule, `ip rule add pref 100 to 10.0.2.0/24 lookup main`, changed `ip route get 10.0.2.2` to `dev eth0 src 10.0.2.15`. The same guest then pinged 10.0.2.2 with 0% loss, and Windows `adb -P 5042 connect 127.0.0.1:5555` returned `connected`; `adb devices -l` showed `device`. An interactive shell read `sys.boot_completed=1`, `KikiAOSP Test Device`, and `7.3.0-rc4-4k`. The diagnostic system image SHA-256 was `f52539924d064877a42f9698961da23b8b60206f8415b9109ef771016a78e583`.
- Moved that route into the canonical device-tree `kiki-adb-wait.sh` before the Kiki TCP daemon starts. Removed temporary ping, logcat, and touch-capture probes while retaining the verified test Activity launch. The clean system-only incremental build succeeded in 26 seconds; local and remote image hashes matched at SHA-256 `770122550da73e1f11988b58ef683ae0ac3b8b48bdffac3501e4b84e37246ea0`. A subsequent local Windows boot of this clean image reached `sys.boot_completed=1` and automatically reconnected ADB as `device` before the Activity's delayed launch. The guest still showed the route at priority 100. The user subsequently confirmed interactive ADB access; the route remains on the feature branch and is included with the Kiki startup-script checkpoint.
- Isolated `-VirtioKeyboard` was added only to the experimental local launcher, leaving the stable launcher untouched. The clean image with `virtio-keyboard-pci` booted; the guest discovered `QEMU Virtio Keyboard` at `/dev/input/event1` (virtio bus 0006, vendor 0627, product 0001). QEMU `sendkey a` produced guest `KEY_A DOWN` and `UP`. Android InputManager initially labeled this device `IsExternal: false` and found no key layout or character map because the minimal image omitted `/system/usr/keylayout` and `/system/usr/keychars`. The canonical device tree now adds an exact-match IDC with `device.internal = 0` plus AOSP's `Generic.kl` and `Generic.kcm`; its build and runtime verification are in progress.
- With working ADB, `wm size` reported 640x480 and DisplayManager exposed only that physical mode. `/sys/class/drm/card0-Virtual-1/modes` lists larger modes including 1920x1080, so dynamic physical resolution remains a HWC/DRM integration task. Audio Framework reported a speaker, but the QEMU run still used the software output stub. Real speaker playback and microphone were not verified.

### W5-KIKI-HOME-APP-LIST-SETTINGS-PROVISIONING-20260926
- Added `HomeActivity` to the existing `KikiWindowTest` APK and registered it as the device's `MAIN`/`HOME`/`DEFAULT` activity. It uses Android's `PackageManager` to enumerate installed `MAIN`/`LAUNCHER` activities, sorts their labels, displays their icons, and launches the selected component. The current guest query returned two entries: `KikiAOSP Window Test` and `Settings`; no separate launcher APK or service was added.
- The boot script now sets `com.kikiaosp.windowtest` as the default HOME before it opens the animated test Activity. `cmd package resolve-activity --brief -a android.intent.action.MAIN -c android.intent.category.HOME` resolved to `com.kikiaosp.windowtest/.HomeActivity`. With this Home installed, the screen visibly rendered `KikiAOSP`, then the installed-app list. This confirms Home draws; the Close-to-Home transition latency still needs an explicit measurement before claiming the five-second delay is fixed.
- Confirmed the Settings row was dispatched by ActivityTaskManager, then matched its immediate exit to the exact log `SettingsHomepageActivity: Device is not provisioned, exiting Settings`. The Kiki product has no SetupWizard, leaving SettingsProvider's AOSP defaults (`def_device_provisioned=false` and `def_user_setup_complete=false`) untouched. Setting both values to `1` in the live test guest allowed Settings to remain open; the foreground view reached the stock “About phone” page. This separates the Settings launch blocker from touch delivery and APK visibility.
- Persisted the headless-test-device setup in the Kiki SettingsProvider resource overlay and in the post-boot script: `device_provisioned=1` and `user_setup_complete=1`. The product also keeps `stay_on_while_plugged_in=15` and selects the Kiki Home before launching the test Activity. A clean-userdata boot of the resulting image pair remains to be verified.
- Builds: `m -j24 KikiWindowTest` succeeded in 13 seconds; the Home-list APK SHA-256 is `4f132ad156cdd3f8a46f57d2ebf7f65ba6f0a94fe90b1f08255ba47bf0f22b4`. A live `adb install -r` succeeded. The following `m -j24 systemimage vendorimage` completed in 25 seconds. Candidate system SHA-256 `7887f0ef96f0908278cbfec4f19d87d06b7a6633339eb70f48f96e2fb7f8cde2`; vendor SHA-256 `b8fed85989fc65ee2f732d917d03efc4a3dcf4034dfa625f5c7fdb62a53fe5bc`. Both were transferred to Windows and matched the host build hashes.
- Full 2880x1920 desktop evidence, captured without maximizing QEMU: `C:\Users\keke\Downloads\temp\qemu-desktop-home-app-list-20260926.png` shows both app rows; `C:\Users\keke\Downloads\temp\qemu-desktop-settings-provisioned-windowed-20260926.png` shows Settings' About phone screen. The user asked to keep QEMU windowed until resolution adaptation; that is now the test constraint. They also observed that resizing briefly blacks the guest display before it recovers; defer that symptom to the resolution/HWC work.
- Current QEMU PID 19956 is left running on the existing windowed display so the user can try selecting Settings and the graphics test. ADB is connected at `127.0.0.1:5555`; the guest reports `mWakefulness=Awake` and `mStayOn=true`. The installed APK update and manual provisioning writes are in the QEMU `-snapshot` runtime; the candidate image pair contains the persistent source changes but has not yet been booted.

### W5-LAUNCHER3-SETTINGS-STABLE-MAIN-20260926
- 用户手动验证 Android 原生 Launcher3/Quickstep 的三键导航、最近任务和通知栏可以交互。一次旧 QEMU 会话中 Quickstep `TouchInteractionService` 的 `notif_expanded` 状态未清除，导致 Recents 命令被拒；关闭旧 QEMU 并冷启动同一产品后状态恢复。不要把该单次异常当成已修好的代码问题，也没有将试验性 Scene Container flag 写入产品配置。
- 收敛产品身份为 `kikiaosp_test_arm64_phone`/`KikiAOSP`，仅将 Launcher3QuickStep、Settings、SystemUI 和必要输入法与 Framework/HAL 依赖装进系统。`KikiWindowTest` 的 APK 源码保留为可选回归用例，但从 `PRODUCT_PACKAGES` 移除。`kiki-adb-wait.sh` 默认 HOME 改为 `com.android.launcher3`，不再自动启动动画测试窗。设备树同步后，两项审计通过。
- 185 开发机 `m -j8 systemimage` 完成，日志为 `/home/keke/projects/kikiaosp_test/build-launcher-settings-stable-20260926.log`，退出码 0；构建输出明确清除了不再安装的 `system/app/KikiWindowTest/KikiWindowTest.apk`。系统镜像 `system-kikiaosp-launcher3-settings-stable-20260926.img` 大小 1,041,625,088 字节，SHA-256 `002e67758ff9cec0cc7c31161ba3cf12be3fad7a8fdfd0e6e4c559dcc830c85e`，Windows 传输后哈希一致。
- Windows ARM 本机 QEMU/WHPX 使用 rc4 4K 内核、同一 vendor 镜像、触摸设备和 virtio 外接键盘冷启动：ADB 返回 `sys.boot_completed=1`；HOME 解析为 `com.android.launcher3/.uioverrides.QuickstepLauncher`；安装包列表有 Launcher3/Settings，没有 `com.kikiaosp.windowtest`。从 Settings 触发 Recents 后，日志 `OverviewCommandHelper: command executed successfully`，整张 Windows 桌面截图 `C:\Users\keke\Downloads\temp\qemu-desktop-launcher3-settings-stable-overview-20260926.png` 显示真实 Android Overview、Settings 任务卡及底部三键。截图包含桌面其他窗口，仅作本机证据，不上传仓库。
- 同一运行中 `cmd statusbar expand-notifications` 使 Quickstep 状态变为 `notif_expanded|awake|notif_visible`；收起后回到 `allow_gesture|awake`，证明没有留下旧会话的通知栏状态。尚未完成 30 分钟压力测试、动态分辨率、高刷、真实音频播放或完整多点触摸测量，不能据此宣称这些功能已完成。
- AOSP 源码曾分散在多份互相重叠的补丁里，导致新检出无法可靠复现。现在以实测工作树生成唯一的 `patches/aosp-working-tree.patch`，`scripts/apply-aosp-integration.sh` 只应用它与 overlays，`scripts/audit-aosp-integration.sh` 核对全部 173 个 tracked 路径并反向 dry-run；审计通过，另有 5 个 overlay 路径。固化 `android17-release` 的 `manifests/aosp-verified-20260926.xml`，不再错误地称此基线为 AOSP master。
- Windows QEMU 的已测试 ARM64 PE 是上游提交 `5f664cd37aec17e8145aa117d8da68f507edc8f1` 加 GTK 全重绘和 Windows ARM 触摸/编译适配；两个下游补丁已独立保存到本设备仓库。主 Windows 启动仓库 `KikiEmu` 的 `main` 已收敛到 README、`.gitignore`、当前启动和全桌面截图脚本 4 个 tracked 文件，提交 `bbe00e3`，旧 HCS/.NET 工程由 Git 历史保留而不再追踪；镜像和构建目录只留本机。

### W5-THREE-REPOSITORY-IMAGE-DELIVERY-20260926
- 修正职责：`kikiaosp_kernel` 产出 4 KiB 内核；`kikiaosp_test` 产出 Android system/vendor 并记录固定 manifest；`KikiEmu` 主仓库保存 QEMU 两个下游补丁、ARM64 构建脚本、镜像清单、SSH 收集/校验/打包及 Windows 启动脚本。之前把 QEMU 补丁放设备树仓库不符合边界，现迁至主仓库；设备树 Git 中删除其副本。
- 内核仓库中已编译但未提交的 rc4 legacy netfilter 选项补入 `flake.nix`，Nix 重新求值得到与测试产物相同 store 输出；`result/boot/kernel` SHA-256 `289e12b89f54b143e253e3aaa4411ffc6f681ad5dfbdd2fd73f0e217629d0d45`，大小 35,613,184 字节。内核主分支提交 `b43c32b66e891105beb47f2c1c04998ac2d845f9`。
- 185 当前 `out/target/product/kikiaosp_test/vendor.img` 与此前 Windows 冻结 vendor 不是同一文件。将当前产物（94,035,968 字节，SHA-256 `be0725a5d239c35280e3c067d22ed0edf59d2774ab966b31d96e16cb97b591be`）传到 Windows，配合现有 system/ramdisk/kernel 用 QEMU/WHPX 测试：`sys.boot_completed=1`、ADB 可连接、HOME 解析为 QuickstepLauncher；首次显示锁屏，`input keyevent 82` 后进入 Settings，`input keyevent 187` 显示真实多任务任务卡。全桌面本地证据 `C:\Users\keke\Downloads\temp\qemu-desktop-current-vendor-unlocked-overview-20260926.png` 不上传。测试后关闭 QEMU。于是当前 185 system/vendor 可以作为同一交付清单的可用产物。
- Windows 已验证运行还用到一份非当前 AOSP 构建自动产出的 ramdisk 与空分区/F2FS userdata/misc。为避免假装可由 `m systemimage vendorimage` 生成，将精确已验证的六个辅助文件归档成 21,641,351 字节的 `kikiaosp-runtime-support-20260926.tar.zst`，SHA-256 `a8073fe77fc1e8a4e52f24af776955d6d9eae4ef5bd848d9dc703d8d4f1c363b`，存放在开发机设备树仓库未追踪的 `output/`。主仓库的收集脚本把这份冻结辅助资产与远端 system/vendor/kernel 一起校验打包；换构建机时必须携带该归档，源码级重新生成完全相同 ramdisk 的配方尚未完成。
