...
- Separate Windows GTK issue: QEMU's GTK/Cairo source image and the QEMU monitor dump contained the complete Activity, but ordinary dirty-rectangle updates intermittently left only the moving square visible in the native window. Added a temporary, environment-gated local QEMU test hook (`KIKI_GTK_FULL_REDRAW=1`) to invalidate the whole GTK window on each display update. With that hook, two 2880x1920 full-desktop Windows captures 18 seconds apart both show the Android Activity, Close button, `TEST OK`, subtitle, and the square at different horizontal positions (animation evidence): `C:\Users\keke\Downloads\temp\qemu-desktop-ui-full-redraw-frame1-20260926.png` and `C:\Users\keke\Downloads\temp\qemu-desktop-ui-full-redraw-frame2-20260926.png`.
- This establishes that the native Android Activity can be rendered in the local Windows QEMU window and observed in multiple frames; it is not a 30-minute stability claim. The QEMU test process was stopped after evidence capture and local ports 4447/5555 were cleared. Audio and microphone integration remain deferred unless a future UI-critical kernel rebuild gives a convenient opportunity to add the option.
